...
class UsersRepository extends Repository {
    ...
    bindAvatarURL(user){
        if (user) {
            let bindedUser = {...user};
            if (user["AvatarGUID"] != ""){
                bindedUser["AvatarURL"] = "http://" + this.req.headers["host"] + NewsFilesRepository.getImageFileURL(user["AvatarGUID"]);
            } else {
    ...
    add(user) {
        ...
        if (User.valid(user)) {
            user["AvatarGUID"] = NewsFilesRepository.storeImageData("", user["ImageData"]);
    ...
    update(user) {
        ...
        if (User.valid(user)) {
            ...
            if (foundUser != null) {
                ...
                user["AvatarGUID"] = NewsFilesRepository.storeImageData(user["AvatarGUID"], user["ImageData"]);
                delete user["ImageData"];
    ...
    remove(id){
        ...
        if (foundUser) {
            NewsFilesRepository.removeImageFile(foundUser["AvatarGUID"]);
